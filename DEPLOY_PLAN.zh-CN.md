...
此目录从 LibreChat 官方 GitHub 仓库的已验证 `v0.8.7` 标签取得，源码提交为 `9e74cc0e57b395926122bd4062c1fcedc48ed465`。本地版本采用“官方三段版本 + 本地修订号”，当前为 `v0.8.7.1`；版本与官方镜像 digest 集中记录在 `deployment/version.env`。运行时以官方同版本、同一 AMD64 manifest digest 的镜像为基础，而不是 `latest`。
...
构建脚本从 `deployment/version.env` 生成 `librechat-local:v0.8.7.1`，在镜像内注入同一界面版本，并重编译 data-provider、API 包和浏览器客户端。每次发布新的本地修改前递增 `LIBRECHAT_LOCAL_REVISION`，不要覆盖已经存在的镜像标签。随后执行 `./scripts/preflight.sh` 和 `./scripts/deploy.sh`；部署脚本会识别本地镜像，不会重新拉取并覆盖它。
