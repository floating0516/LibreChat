...
export default function Landing({ centerFormOnLanding }: { centerFormOnLanding: boolean }) {
  ...
  return (
    <div
      className={`chat-landing flex h-full transform-gpu flex-col items-center justify-center pb-16 transition-all duration-200 ${centerFormOnLanding ? 'max-h-full sm:max-h-0' : 'max-h-full'} ${getDynamicMargin}`}
    >
          ...
          {((isAgent || isAssistant) && name) || name ? (
            <div className="flex flex-col items-center gap-0 p-2">
              <SplitText
                key={`split-text-${name}`}
                text={name}
                className={`${getTextSizeClass(name)} landing-title font-medium text-text-primary`}
                delay={50}
                textAlign="center"
            ...
            <SplitText
              ...
              text={greetingText}
              className={`${getTextSizeClass(greetingText)} landing-title font-medium text-text-primary`}
              delay={50}
              textAlign="center"
              animationFrom={{ opacity: 0, transform: 'translate3d(0,50px,0)' }}
              animationTo={{ opacity: 1, transform: 'translate3d(0,0,0)' }}
